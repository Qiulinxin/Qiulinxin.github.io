...
这张图是我日常工作和写东西时所的流程，包含了相应的软件和方法，括号内为涉及到的文件格式。这篇博客是记录如何从 `Jupyter Notebook(.ipynb)` 到 `Markdown(.md)`，从 `Markdown(.md)` 到 `Word(.docx)` 的过程已在另一篇博客 [《Word半自动排版》](https://qiulinxin.github.io/2023-02/Word%E5%8D%8A%E8%87%AA%E5%8A%A8%E6%8E%92%E7%89%88) 中记录。

### 应用场景

在 `Jupyter Notebook` 中使用 `Python` 编写计算书完成计算后，将 `Jupyter Notebook`文件导出为 `Markdown` 文件进而转换为 `Word` 可以自动完成约 80% 的排版任务，节省了大量时间。最为节省时间的是`Jupyter Notebook` 可以自动生成 `LaTeX` 公式代码，生成的公式仍然可以在 `Word` 中使用，避免了在 `Word` 中手工输公式的痛苦过程。[^1]

### 功能实现

在实际使用 `Jupyter Notebook` 编写计算书时会涉及编写多个 `Jupyter Notebook` 文件，最终成果需将 `Jupyter Notebook` 文件批量转换为 `Markdown` 文件[^2]并在去除 `Python代码` 后合并为一个单独的 `Markdown` 文件。

脚本目前已实现以下功能：

1.   将 `Jupyter Notebook` 文件批量转换为 `Markdown` 文件[^3]；
2.   将转换后的 `Markdown` 文件去除 `Python` 代码后再合并为一个文件。
